...
class PrimeAccessRepository(
    ...
    fun isOnAllowList(userId: String): Boolean = primeAccessList.entries
        .any { entry -> entry.userId == userId && entry.allowList }
    fun isOnDenyList(userId: String): Boolean = primeAccessList.entries
        .any { entry -> entry.userId == userId && entry.denyList }
}

class PrimeAccessRepositoryByAssociate(
    private val primeAccessList: PrimeAccessList
) {
    private val entries = primeAccessList.entries.associateBy { it.userId }
...
fun main() {
    ...
    val repoByAssociate: PrimeAccessRepositoryByAssociate
    measureTimeMillis {
        repoByAssociate = PrimeAccessRepositoryByAssociate(accessList)
    }.also { println("Class creation took $it ms") }

    measureTimeMillis {
        for (userId in 1L..10_000L) {
            repoByAssociate.isOnAllowList(userId.toString())
        }
    }.also { println("Operation took $it ms") }

    measureTimeMillis {
        for (userId in 1L..10_000L) {
            repoByAssociate.isOnDenyList(userId.toString())
        }
    }.also { println("Operation took $it ms") }
}